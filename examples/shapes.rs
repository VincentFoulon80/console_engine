...
fn main() {
    ...
    loop {
        ...
        if (engine.is_key_held(Key::Char('8')) || engine.is_key_pressed(Key::Up)) && coords[selection].1 > 0 && (selection == 0 || shape != Shapes::Circle) {
            coords[selection].1 -= 1;
        }
        if (engine.is_key_held(Key::Char('6')) || engine.is_key_pressed(Key::Right)) && coords[selection].0 < engine.get_width() as i32-1 {
            coords[selection].0 += 1;
        }
        if (engine.is_key_held(Key::Char('2')) || engine.is_key_pressed(Key::Down)) && coords[selection].1 < engine.get_height() as i32-1 && (selection == 0 || shape != Shapes::Circle) {
            coords[selection].1 += 1;
        }
        if (engine.is_key_held(Key::Char('4')) || engine.is_key_pressed(Key::Left)) && coords[selection].0 > 0 {
            coords[selection].0 -= 1;
        }
